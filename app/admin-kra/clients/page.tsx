...
export default function FeaturesPage() {
  ...
  const fetchFeatures = async () => {
    try {
      setLoading(true)
      const response = await fetch(`https://api.smartcorpweb.com/api/client?OrgCode=${orgCode}`)
      if (response.ok) {
      ...
      toast({
        title: "Error",
        description: "Failed to fetch Clientss",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure you want to delete this Clients?")) return

    try {
      const response = await fetch(`https://api.smartcorpweb.com/api/client/${id}?OrgCode=${orgCode}`, {
        method: "DELETE",
      })
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Clients ({filteredFeatures.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
                ...
                <TableRow key={feature.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link href={`/admin-kra/clients/edit/${feature.id}`}>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground mb-4">
                {searchTerm || statusFilter !== "all"
                  ...
                  : "Create your first Clients to get started"}
              </p>
              {!searchTerm && statusFilter === "all" && (
                <Link href="/admin-kra/clients/create">
                  <Button>
