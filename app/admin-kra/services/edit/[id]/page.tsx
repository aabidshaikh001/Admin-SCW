...
export default function EditFeaturePage({ params }: { params: { id: string } }) {
  ...
  const fetchFeature = async () => {
    try {
      const response = await fetch(`https://api.smartcorpweb.com/api/services/${params.id}?OrgCode=${orgCode}`)
      if (response.ok) {
...
if (feature.Img) {
 ...
 const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  if (value !== undefined && value !== null) {
    // convert boolean to "1"/"0", number to string
...
if (file) {
  form.append("Img", file)
      ...
      const response = await fetch(
        `https://api.smartcorpweb.com/api/services/${params.id}?OrgCode=${orgCode}`,
        {
          method: "PUT",
          body: form,
        }
