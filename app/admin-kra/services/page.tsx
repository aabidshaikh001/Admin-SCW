...
import { Plus, Search, Edit, Trash2, Eye, EyeOff, Palette,ImageIcon } from "lucide-react"
...
export default function FeaturesPage() {
  ...
  const fetchFeatures = async () => {
    try {
      setLoading(true)
      const response = await fetch(`https://api.smartcorpweb.com/api/services?OrgCode=${orgCode}`)
      if (response.ok) {
      ...
      toast({
        title: "Error",
        description: "Failed to fetch Servicess",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure you want to delete this Services?")) return

    try {
      const response = await fetch(`https://api.smartcorpweb.com/api/services/${id}?OrgCode=${orgCode}`, {
        method: "DELETE",
      })
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage your KRA Services and their display settings</p>
        </div>
         <div className="flex items-center gap-2">
            {/* ✅ Preview All Dialog Trigger */}
            <Dialog>
              ...
              <DialogContent className="max-w-4xl">
                <DialogHeader>
                  <DialogTitle>Preview All Services</DialogTitle>
                </DialogHeader>
  ...
  {features.map((feature) => (
    ...
    >
...
</div>   {/* ✅ now closed properly */}

              </DialogContent>
            </Dialog>

            {/* Add Services Button */}
            <Link href="/admin-kra/services/create">
              <Button>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Services ({filteredFeatures.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
                ...
                <TableRow key={feature.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link href={`/admin-kra/services/edit/${feature.id}`}>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground mb-4">
                {searchTerm || statusFilter !== "all"
                  ...
                  : "Create your first Services to get started"}
              </p>
              {!searchTerm && statusFilter === "all" && (
                <Link href="/admin-kra/services/create">
                  <Button>
