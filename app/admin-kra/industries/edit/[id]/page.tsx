...
export default function EditFeaturePage({ params }: { params: { id: string } }) {
  ...
  const fetchFeature = async () => {
    try {
      const response = await fetch(`https://api.smartcorpweb.com/api/industry/${params.id}?OrgCode=${orgCode}`)
      if (response.ok) {
...
if (feature.Img) {
  setPreview(`https://api.smartcorpweb.com${feature.Img}`)
      ...
      toast({
        title: "Error",
        description: "Failed to fetch Industries details",
        variant: "destructive",
      })
      router.push("/admin-kra/industries")
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.smartcorpweb.com/api/industry/${params.id}?OrgCode=${orgCode}`,
        {
          method: "PUT",
          body: form,
        }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <Link href="/admin-kra/industries">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Edit Industries</h1>
            <p className="text-muted-foreground">Update the KRA Industries details and styling</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              <CardTitle>Industries Details</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex gap-2 pt-4">
                  <Button type="submit" disabled={loading}>
                    ...
                    {loading ? "Updating..." : "Update Industries"}
                  </Button>
                  <Link href="/admin-kra/industries">
