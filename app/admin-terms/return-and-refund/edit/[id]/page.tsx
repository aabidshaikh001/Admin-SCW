...
export default function EditTOSPage({ params }: { params: { id: string } }) {
  ...
  try {
    ...
    const response = await fetch(`https://api.smartcorpweb.com/api/returns/${orgCode}/${params.id}`)
    ...
    toast({
      title: "Error",
      description: "Failed to fetch Return & Refund item",
      variant: "destructive",
    })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: "Return & Refund item updated successfully",
        })
        router.push("/admin-terms/return-and-refund")
      } else {
        throw new Error("Failed to update Return & Refund item")
      }
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to update Return & Refund item",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Edit Return & Refund</h1>
            <p className="text-muted-foreground">Update Return & Refund item details</p>
          </div>
        </motion.div>
