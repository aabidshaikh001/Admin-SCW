...
export default function CreateTOSPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      const response = await fetch("https://api.smartcorpweb.com/api/shipping", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: "Shipping Terms item created successfully",
        })
        router.push("/admin-terms/shipping")
      } else {
        throw new Error("Failed to create Shipping Terms item")
      }
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to create Shipping Terms item",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Create Shipping Terms</h1>
            <p className="text-muted-foreground">Add a new Shipping Terms item</p>
          </div>
        </motion.div>
