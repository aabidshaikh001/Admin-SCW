...
export default function EditTOSPage({ params }: { params: { id: string } }) {
  ...
  try {
    ...
    const response = await fetch(`https://api.smartcorpweb.com/api/payment/${orgCode}/${params.id}`)
    ...
    toast({
      title: "Error",
      description: "Failed to fetch Payment Terms item",
      variant: "destructive",
    })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: "Payment Terms item updated successfully",
        })
        router.push("/admin-terms/payment")
      } else {
        throw new Error("Failed to update Payment Terms item")
      }
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to update Payment Terms item",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
