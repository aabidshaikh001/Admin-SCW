...
export default function TOSPage() {
 ...
 const fetchTosItems = async () => {
  try {
    ...
    const response = await fetch(`https://api.smartcorpweb.com/api/payment/${orgCode}`)
 ...
 const filterTosItems = () => {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure you want to delete this Payment Terms item?")) return

    try {
      setDeleting(id)
      const orgCode = user?.OrgCode || 1 // Replace with actual org code from auth
      const response = await fetch(`https://api.smartcorpweb.com/api/payment/${orgCode}/${id}`, {
        method: "DELETE",
      })
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: "Payment Terms item deleted successfully",
        })
        fetchTosItems()
      } else {
      ...
      toast({
        title: "Error",
        description: "Failed to delete Payment Terms item",
        variant: "destructive",
      })
    } finally {
      setDeleting(null)
      ...
      <div className="space-y-6">
        ...
        >
          <div>
            ...
            <p className="text-muted-foreground">Manage your Payment Terms content</p>
          </div>
          <Link href="/admin-terms/payment/create">
            <Button>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Payment Terms Items ({filteredTosItems.length})</CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold text-muted-foreground">No Payment Terms found</h3>
                <p className="text-sm text-muted-foreground mb-4">
                  {searchTerm || statusFilter !== "All"
                    ? "Try adjusting your search or filters"
                    : "Get started by creating your first Payment Terms item"}
                </p>
                <Link href="/admin-terms/payment/create">
                  <Button>
                    <Plus className="w-4 h-4 mr-2" />
                      ...
                      >
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end space-x-2">
                            <Link href={`/admin-terms/payment/edit/${item.id}`}>
