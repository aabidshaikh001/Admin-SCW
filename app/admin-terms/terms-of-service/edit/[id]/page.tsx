...
export default function EditTOSPage({ params }: { params: { id: string } }) {
  ...
  const fetchTOS = async () => {
    try {
      ...
      const response = await fetch(`https://api.smartcorpweb.com/api/tos/${orgCode}/${params.id}`)

      if (response.ok) {
        const data: TOS = await response.json()
        setFormData({
      ...
      toast({
        title: "Error",
        description: "Failed to fetch terms of service item",
        variant: "destructive",
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: "Terms of service item updated successfully",
        })
        router.push("/admin-terms/terms-of-service")
      } else {
        throw new Error("Failed to update terms of service item")
      }
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to update terms of service item",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Edit Terms of Service</h1>
            <p className="text-muted-foreground">Update terms of service item details</p>
          </div>
        </motion.div>
