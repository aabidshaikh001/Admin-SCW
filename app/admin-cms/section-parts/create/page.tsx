...
export default function CreateSectionPartPage() {
      const { user } = useAuth()
...
const [loading, setLoading] = useState(false)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("https://api.smartcorpweb.com/api/section-parts", {
        method: "POST",
        body: submitData,
      })
      ...
      if (response.ok) {
        ...
        throw new Error("Failed to create section part")
      }
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to create section part",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
      ...
      {formData.btn2Text}
              ...
              <div className="flex gap-4">
                <Button type="submit" disabled={loading}>
                  ...
                  {loading ? "Creating..." : "Create Section Part"}
                </Button>
                <Link href="/admin-cms/section-parts">
