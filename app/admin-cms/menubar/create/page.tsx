...
export default function CreateMenuPage() {
 ...
 const [mainMenus, setMainMenus] = useState<MainMenu[]>([])
  ...
  try {
    ...
    submitData.append("OrgCode", formData.OrgCode.toString())
    submitData.append("name", formData.name)
    submitData.append("type", formData.type)

    // Only send parentId for sub menus
    ...
    // ✅ Only send isButton & buttonColor if enabled
    if (formData.isButton) {
      submitData.append("isButton", "true")
      submitData.append("buttonColor", formData.buttonColor)
    } else {
      submitData.append("isButton", "false")
    }

    submitData.append("isActive", formData.isActive ? "true" : "false")

    const response = await fetch("https://api.smartcorpweb.com/api/menu", {
      method: "POST",
      body: submitData,
    })
    ...
    if (response.ok) {
      ...
      alert(`Error: ${error.message || "Failed to create menu item"}`)
    }
  } catch (error) {
